...
class DatedUpdateHolder extends Page {
	...
	/**
	 * Find all distinct tags (TaxonomyTerms) associated with the DatedUpdatePages under this holder and child holders.
	 */
	...
	/**
	 * Wrapper to find all updates belonging to this holder and child holders, based on some filters.
	 */
	public function Updates($tagID = null, $dateFrom = null, $dateTo = null, $year = null, $monthNumber = null) {
		...
		return static::AllUpdates($className, $this->getChildHolderIDs(), $tagID, $dateFrom, $dateTo, $year, $monthNumber);
	}



	/**
	 * Gets an array of ids of all the children of this page that are DatedUpdateHolders
	 * @return array
	 **/
	public function getChildHolderIDs(){
		$stack[] = $this->ID;
		
		$childHolders = DatedUpdateHolder::get()->filter('ParentID', $this->ID);
		if($childHolders->count()){
			foreach ($childHolders as $childHolder) {
				$stack = array_merge($stack, $childHolder->getChildHolderIDs());
			}
		}
